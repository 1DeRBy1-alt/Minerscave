...
local Config = {
	KillAura = { Enabled = false, Delay = 12, Range = 16.5 },
...
local function getEquippedBlockSlot(character)
	local selectedSlotVal = character:FindFirstChild("SelectedSlot")
	if not selectedSlotVal then return nil end
	local inventory = character:FindFirstChild("Inventory")
	if not inventory then return nil end
	local activeSlotId = selectedSlotVal.Value
	local slot = inventory:FindFirstChild("Slot" .. activeSlotId)
	if slot then
		local success, data = pcall(HttpService.JSONDecode, HttpService, slot.Value)
		if success and data and data.count and data.count > 0 and data.name then
			local item = M_ItemInfo[data.name]
			if item and item.block then
				return activeSlotId, data.name
			end
		end
	end
	return nil
...
local function isWater(part)
	return (part.Name == "Water" or part.Name == "Lava")
end
...
local NoFallMod = MovementWindow:AddModule("No Fall", "Toggles fall damage.", function(state)
	...
	local demo = GameRemotes:FindFirstChild("Demo") or GameRemotes:FindFirstChild("NoDemo")
	if demo then
		demo.Name = state and "NoDemo" or "Demo"
	end
end)
...
AirPlaceMod:AddSlider("Range", 1, 20, 6, 1, function(val)
...
task.spawn(function()
	while task.wait(Config.Triggerbot.Delay) do
		...
		if hum and hum.Health > 0 and Players:GetPlayerFromCharacter(targetModel) then
			task.spawn(function()
...
task.spawn(function()
	...
	scaffoldConnection = RunService.PostSimulation:Connect(function()
		...
		local blockSlot, itemName = getEquippedBlockSlot(char)
		if not blockSlot or not itemName then return end
		
		local blockData = BlocksByName[itemName]
		local blockId = blockData and blockData.id
		if not blockId then return end
		
		local centerPos = root.Position + Vector3.new(0.5, -4.5, 0.5)
		
		local cx, cy, cz = worldToBlock(centerPos.X, centerPos.Y, centerPos.Z)
		local extent = math.floor(Config.Scaffold.Range / 2)
		
		local targets = {}
		for dx = -extent, extent do
			for dz = -extent, extent do
				local tx, ty, tz = cx + dx, cy, cz + dz
...
task.spawn(function()
	...
	RunService.RenderStepped:Connect(function()
		...
		local blockSlot, itemName = getEquippedBlockSlot(LocalPlayer.Character)
		if not blockSlot or not itemName then
			previewPart.Parent = nil
			return
		end
